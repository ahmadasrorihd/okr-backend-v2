...
class M_Presence extends CI_Model
{
    ...
    public function get_status_presence($id)
    {
        $where = 'user_id="' . $id . '" and clock_in=(select max(clock_in) from t_presence where user_id="' . $id . '")';
        $q  = $this->db->select('id, user_id, clock_in, created_date, status')->from('t_presence')->where($where)->get()->row();
        if ($q == "") {
            return array('status' => 200, 'message' => 'Belum ada data absen.', 'user_id' => $id, 'status' => 'out');
        } else {
            $id             = $q->id;
            $user_id             = $q->user_id;
            $clock_in             = $q->clock_in;
